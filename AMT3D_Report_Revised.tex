...
Three-Dimensional Extension Study}
...
The AMT+ (Acoustic Multi-Target Plus) system \cite{amt_plus} established a foundation for acoustic tracking using smartphone-based MIMO (Multiple-Input Multiple-Output) arrays. The original system operates in 2D space with the following key characteristics:

\begin{itemize}
\item {\em 2D State Vector:} Tracks position $(x, y)$, velocity $(v_x, v_y)$, and acceleration $(a_x, a_y)$ in a 6-dimensional state space
\item {\em Circle Intersection:} Localizes targets by finding intersections of circles defined by acoustic path lengths
\item {\em Planar Geometry:} Assumes all microphones and targets operate in the same horizontal plane
\item {\em Kalman Filtering:} Uses linear Kalman filters adapted for 2D motion models
\item {\em Multi-target Capability:} Simultaneously tracks multiple acoustic sources with target identification
\end{itemize}

The AMT+ system achieved centimeter-level accuracy in controlled 2D environments by leveraging:

\begin{itemize}
\item {\em Time Difference of Arrival (TDOA):} Precise measurement of acoustic signal arrival times
\item {\em Echo Cancellation:} Advanced algorithms to separate direct path from multipath reflections
\item {\em Target Association:} Probabilistic data association for multi-target scenarios
\item {\em Room Acoustic Modeling:} Compensation for environmental acoustic properties
\end{itemize}
...
\item {\em State Space Expansion:} Moving from 6D to 9D state vectors increases filter complexity and computational requirements
...
One of the primary challenges in 3D extension is the limited geometric diversity in the vertical dimension. Traditional microphone arrays are typically deployed horizontally, creating strong geometric constraints for z-axis localization.

{\bf Problem:} With microphones primarily in horizontal planes, the acoustic baselines for vertical triangulation are significantly shorter than horizontal baselines, leading to:
...
{\bf Challenge:} The original AMT+ system uses a 6-dimensional state vector for 2D tracking. Extending to 3D requires careful redesign of the state space and motion models.

{\bf Solution:} We implement a 9-dimensional state vector to track position, velocity, and acceleration in all three dimensions:
...
This expansion from 6D to 9D increases computational complexity but provides the necessary degrees of freedom for full 3D motion modeling.

\subsection{Multi-Ellipsoid Intersection Algorithm}

{\bf Challenge:} The AMT+ 2D system solves circle intersection problems. In 3D, we must solve ellipsoid intersection problems, which are significantly more complex both geometrically and computationally.
...
\caption{{\it Multi-ellipsoid intersection visualization showing acoustic ellipses converging on target position. Each speaker-microphone pair contributes a constraint ellipse, and the target location is determined through optimization of the intersection problem.}}
\label{fig:ellipsoid_intersection}
\end{figure}

{\bf Solution Implementation:} We implement an iterative optimization approach using:

\begin{itemize}
\item {\em Gradient descent optimization:} For finding optimal intersection points
\item {\em Weighted least squares:} To handle measurement uncertainty
\item {\em Robust estimation:} To reject outlier measurements from multipath effects
\end{itemize}
...
Additionally, we introduce temporal continuity constraints to prevent unrealistic vertical position jumps:
...
where $\alpha_z$ is a weighting factor that penalizes sudden vertical movements while preserving legitimate motion.
...
Figure \ref{fig:system_architecture} illustrates the overall AMT3D system architecture.

\begin{figure}[t]
\centerline{
\includegraphics[width=85mm]{final/system_architecture.png}}
\caption{{\it AMT3D system architecture showing the 3D extension components including enhanced state modeling, multi-ellipsoid intersection algorithms, and adaptive noise handling for vertical measurements.}}
...
Figure \ref{fig:snr_analysis} shows the tracking error performance across different noise conditions, demonstrating both mean and maximum error bounds.

\begin{figure}[t]
\centerline{
\includegraphics[width=85mm]{final/snr_vs_error_focused.png}}
\caption{{\it Mean and maximum 3D tracking error versus noise levels, showing the system's performance degradation under challenging acoustic conditions. Both error metrics improve consistently as noise decreases, validating the robustness of the 3D extension.}}
\label{fig:snr_analysis}
\end{figure}

\subsection{Error Characteristics Analysis}

Figure \ref{fig:error_metrics} provides detailed analysis of different error metrics across noise conditions.

\begin{figure}[t]
\centerline{
\includegraphics[width=85mm]{final/error_metrics_comparison.png}}
\caption{{\it Error metrics comparison showing mean, standard deviation, and 95th percentile tracking errors across different noise levels. The analysis reveals consistent behavior across all error measures, indicating stable algorithm performance.}}
\label{fig:error_metrics}
\end{figure}

\subsection{Temporal Tracking Stability}

A critical aspect of 3D tracking is maintaining stability over extended time periods. Figure \ref{fig:temporal_tracking} demonstrates temporal performance characteristics.

\begin{figure}[t]
\centerline{
\includegraphics[width=85mm]{final/temporal_tracking_performance.png}}
\caption{{\it Temporal tracking performance showing error evolution over time for different noise levels. Higher SNR conditions demonstrate more stable tracking with reduced variance over extended sequences, validating the temporal continuity constraints implementation.}}
...
This work successfully demonstrates the extension of acoustic multi-target tracking from 2D to 3D space. The key contributions include:

\subsection{Technical Achievements}

\begin{itemize}
\item {\em 9D state space implementation:} Enhanced Kalman filtering for full 3D motion tracking
\item {\em Multi-ellipsoid intersection:} Robust algorithms for 3D position estimation from acoustic measurements
\item {\em Adaptive noise handling:} Recognition and compensation for z-axis measurement challenges
\item {\em Temporal continuity:} Constraints to improve vertical tracking stability
\end{itemize}

\subsection{Performance Validation}

The 3D extension maintains practical tracking accuracy with mean errors under 0.2 m across various conditions, demonstrating feasibility for real-world applications requiring full 3D acoustic tracking.
...
\begin{itemize}
\item {\em Adaptive array configuration:} Automatic optimization of microphone placement for 3D scenarios
\item {\em Machine learning integration:} Neural networks for improved multipath handling and target association
\item {\em Real-time optimization:} Further computational efficiency improvements for real-time deployment
\item {\em Multi-room extension:} Scaling to larger 3D environments with multiple interconnected spaces
\end{itemize}

The AMT3D system establishes a foundation for practical 3D acoustic tracking, opening new possibilities for spatial interaction and monitoring applications across diverse 3D environments.
